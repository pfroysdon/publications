...
alphaD = 0.0001;    % Learning rate for the discriminator
alphaG = 0.0001;    % Learning rate for the generator
...
function model = ganTrainCameraman(realData, inputDim, zDim, epochs, alphaD, alphaG, batchSize, imgSize)
    ...
    for epoch = 1:epochs
        ...
        fakeData = w_G * Z + repmat(b_G', 1, batchSize);  % size: [inputDim x batchSize]
...
%     z = randn(model.noiseDim, numSamples);  % [zDim x numSamples]
%     % Generator forward pass:
%     h1 = max(model.w_G1 * z + repmat(model.b_G1, 1, numSamples), 0);  % [d_hidden_G x numSamples]
%     z2 = model.w_G2 * h1 + repmat(model.b_G2, 1, numSamples);           % [inputDim x numSamples]
%     fakeFlat = sigmoid(z2);                                             % [inputDim x numSamples]
...
%     generatedImages = reshape(fakeFlat', H_img, W_img, numSamples);
% end

function generatedImages = ganPredict(model, numSamples)
    z = randn(model.noiseDim, numSamples);  % [zDim x numSamples]
    % One-layer generator forward pass:
    z2 = model.w_G * z + repmat(model.b_G', 1, numSamples);  % [inputDim x numSamples]
    fakeFlat = sigmoid(z2);  % [inputDim x numSamples]
    fakeFlat = fakeFlat';    % [numSamples x inputDim]
...
function d = leakyReluDerivative(x, alpha)
    % leakyReluDerivative returns the derivative of the Leaky ReLU activation.
    % For x >= 0, derivative is 1; for x < 0, derivative is alpha.
    d = ones(size(x));
    d(x < 0) = alpha;
end
