...
paramsGen.FCb4 = dlarray(zeros(prod(settings.image_size),1,'single'));
...
% Train
%-------------------------------------------------------------------------%
numIterations = 1000; %floor(size(trainX,2)/settings.batch_size);
out = false;
epoch = 0;
global_iter = 0;
...
while ~out
    ...
    for i=1:numIterations
        ...
        noise = gpdl(randn([settings.latent_dim, settings.batch_size]),'CB');
        
        % automatic differentiation tracing
        [GradGen,GradDis,stGen,stDis] = dlfeval(@modelGradients, XBatch, ...
            noise, paramsGen,paramsDis,stGen,stDis);
        
        % Update Discriminator network parameters via adaptive moment estimation
        [paramsDis,avgG.Dis,avgGS.Dis] = adamupdate(paramsDis, GradDis, ...
            avgG.Dis, avgGS.Dis, global_iter, settings.lrD, settings.beta1, settings.beta2);
        
        % Update Generator network parameters via adaptive moment estimation
        [paramsGen,avgG.Gen,avgGS.Gen] = adamupdate(paramsGen, GradGen, ...
            avgG.Gen, avgGS.Gen, global_iter, settings.lrG, settings.beta1, settings.beta2);
        ...
        if i==1 && epoch==0
            % save_all_figs_OPTION('results/gan_iter1_epoch0','png',1)
        end
        if i==100 && epoch==0
            % save_all_figs_OPTION('results/gan_iter100_epoch0','png',1)
        end
    end
...
% save_all_figs_OPTION('results/gan_iter1000_epoch49','png',1)
